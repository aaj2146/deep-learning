...
def conv2d_forward(x, w, b, pad, stride):
    ...
    #                                                                     #
    #######################################################################
    (batch_size, data_height, data_width, channels) = x.shape
    (num_of_filters, filter_height, filter_width, channels) = w.shape
    #print(channels)
    
    output_height = ((data_height - filter_height + 2 * pad) // stride) + 1
    output_width = ((data_width - filter_width + 2 * pad) // stride) + 1
    
    #print(output_height)
    #print(output_width)
    
    
    padded = np.pad(x, [(0,0),(pad,pad), (pad,pad), (0,0)], 'constant')

    #print(padded.shape)
    out = np.zeros((batch_size, output_height, output_width, num_of_filters))
        ...
        for j in range(num_of_filters):
            ...
            for k in range(output_height):
                ...
                for l in range(output_width):
                    ...
                    window = padded[i, hs:hs+filter_height, ws:ws+filter_width,:]
                    
                    out[i, k, l, j] = np.sum(w[j]*window) + b[j]
...
def conv2d_backward(d_top, x, w, b, pad, stride):
    ...
    new_height = ((height - filter_height + 2 * pad) // stride) + 1
    new_width = ((width - filter_width + 2 * pad) // stride) + 1
    height_new=d_top.shape[1]
    width_new=d_top.shape[2]
       
    out=np.zeros((batch,new_height,new_width,num_of_filters))
    d_w=np.zeros((num_of_filters,filter_height,filter_width,channels))
    d_b=np.zeros((num_of_filters))
    padded_x = np.pad(x,((0,),(pad,),(pad,),(0,)),'constant')
   
    for nf in range(num_of_filters):
        d_b[nf] = np.sum(d_top[:,:,:,nf])

    for nf in range(num_of_filters):
        for ch in range(channels):
            for h in range(filter_height):
                for w in range(filter_width):
                    padded_x_subset = padded_x[:,h:h+new_height*stride:stride,w:w+new_width*stride:stride,ch]
                    d_w[nf,h,w,ch] = np.sum(d_top[:, :, :,nf]*padded_x_subset)
    print('d_w shape is',d_w.shape)
    print('d_b shape is',d_b.shape)
    return d_w, d_b
    #raise NotImplementedError
